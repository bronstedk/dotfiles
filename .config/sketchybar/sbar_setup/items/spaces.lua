...
local function updateWindows(env)
    local spacesWithNames = GetWindowsAndSpaces()
    local currSpace = GetCurrentSpace()

    for space, _ in pairs(spacesWithNames) do
...
for i, spaceID in ipairs(tempSpaces) do
    ...
    space:subscribe("aerospace_workspace_change", spaceSelection)
...
local sep = sbar.add("bracket", spaces, {
    background = { color = colors.accentColor, border_color = colors.accentColor },
})
